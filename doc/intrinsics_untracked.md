...
### 8.1 `{.naked.}` — no prologue, no epilogue

One thing an `.assembler` body still could not say: *where my caller's frame is*.
By the time a proc's first instruction runs, its prologue has already moved SP,
so every location it can name describes its own frame. The fact that is lost is
the one a stack walk needs to start — on entry, before any prologue, SP points
...
proc callerFrame(): pointer {.assembler, naked.} =
  result = stackPointer()          # `mov rax, rsp` — and nothing else
```

It is a pragma rather than a code shape because the prologue is not written in
the body: arkham synthesizes it from what the body USES (`framePush`,
`emitFrameSub`). So "no prologue" cannot be expressed by writing less; it has to
be declared, and then checked.

The checking is three rejections, one per way a naked body could still need the
frame it gave up — each of which would otherwise corrupt the caller silently
rather than fail:

- **an allocated (non-`assembler`) body** — spills go to `(s)` slots, a call
  needs the outgoing-argument area, and the seventh parameter is read relative
  to the frame. `{.naked.}` therefore requires `{.assembler.}`;
- **a `{.stack.}` local** — a slot in the frame that is not there;
- **a callee-saved register** — its `push` never happened, so the caller's value
  in it is destroyed, and the symptom appears arbitrarily far from the cause.

The `ret` stays: "no prologue/epilogue" is a statement about the FRAME. Without a
return the proc would fall into whatever the linker put next.

Two rows serve the rest of the walk, both `icPortable` and both zero-operand
(§4.2's `k = 0` case, which the operand loop cannot express and which
`isMachineQuery` therefore takes before it):

| Row | Lowering | What it is |
|---|---|---|
| `StackPointer` | `mov D, rsp` | the one register a value may be READ out of but never homed in — `asmPinReg` rejects `rsp`/`rbp` as homes because the frame owns them, and copying the value out touches neither |
| `TraceTable` | `lea D, [rip+arkham.traceinfo.0]` | the address of nifasm's per-proc metadata blob; naming it is also what makes nifasm emit it (`doc/tracetable.md`) |

Everything above them — the search, the frame loop, the name formatting — is
ordinary Nim in `nimony/lib/std/stacktraces.nim`. That split is the point: the
back end supplies the two facts a machine knows and a language cannot express,
and not one line of string handling.
...
x86-64 AND on both Arm profiles for the subset listed below, and flags are
reachable on all three. Phases 0, 1,
...
### `{.naked.}` and the stack-trace rows

| Piece | Where |
|---|---|
| The pragma | `(naked)`, appended to `nimony/doc/tags.md`; `sempragmas` records the flag, `hexer/lengcgen` forwards the tag, both C backends refuse it with the same message they give `{.assembler.}` |
| Frame suppression | `arkham/codegen_x64.genAsmProc` — skips `framePush`/`emitFrameSub`/`framePop`, keeps `ret`; the three rejections live there and in `genProc` |
| The rows | `StackPointer`, `TraceTable` in `nimony/src/lib/intrinsics.nim`; `isMachineQuery` is what routes a zero-operand register-valued row past the operand machinery in both `emitInstr2` and `asmInstr` |
| The table | `nifasm/tracetable.nim` (format), `dwarf.collectTraceProcs` (the rows, from the CFI collector's own `ProcUnwind`), `assembler.{appendTraceTable,fillTraceTable}` (reserve before layout, write after) |
| The walk | `nimony/lib/std/stacktraces.nim` — all of it ordinary Nim |

Tests: `tests/arkham/naked_stacktrace_x64.c.nif` (the ELF path, exit code 7 for
magic + table placement + the naked frame), `tests/win_tracetable.c.nif` under
Wine (the PE path, whose writer fills the table at a different point in the
layout), `tests/arkham/err_naked_{alone,stack,callee}.c.nif` (the three
rejections), and `nimony/tests/nimony/stacktraces/tstacktrace.nim` end to end on
both backends.
...
`sf`, `ns`, `of`, `no`, `pf`, `np`): `uses` names the bit, and it is legal
...
No deviations from "the name is the nifasm tag". There used to be two — `of` and
`no` were spelled `ovf`/`novf`, because the pragma argument was an IDENT and a Nim
keyword cannot stand where an expression is expected (the same collision that made
`{.asm.}` impossible). The argument is a string literal now, so `{.instruction:
"of".}` parses and the cover names are gone, along with the mapping the backend
needed to undo them.
...
Twelve rows with `roles[0] == roInout` and no result: `add`, `sub`, `and`, `or`,
`xor`, `shl`, `shr`, `sar`, `neg`, `not`, `inc`, `dec` — the machine's own
mnemonics, keywords included, since a string literal admits them.
...
| Transliteration | `arkham/codegen_x64.genAsmProc` and `arkham/codegen_arm_asm.genAsmProc2` — no `allocateProc`, no analyser, no value core |
| The target-neutral half | `arkham/codegen_common` — `asmDeclSpec`, `asmAtom`, `asmRegOf`, `instrOpAt`, `isResultName`: what a location pragma says and what counts as an atom is the same question on every target, so both modes read one answer |
...
**The Arm targets** (`arkham/codegen_arm_asm.nim`, included by `codegen_arm.nim`)
serve the same subset, on both profiles: AArch64 pins are `x0`..`x30`, Cortex-M's
are `r0`..`r12`, and everything that differs between the two beyond the spelling
is already a fact in `g.md` — the argument registers, the return register, the
callee-saved homes the prologue saves.

What the Arm mode adds to the shared rules is a pin check with the machine's own
reasons in it. A register may be claimed only if the ABI lets a callee destroy it
or this back end's prologue saves it; everything else is refused BY ROLE, naming
what already lives there — Cortex-M's r12 (nifasm's operand-folding scratch,
written at sites arkham never sees), r8/r9 (the produce bridge and the indirect
result pointer), r10/r11 (the two staging bridges), AArch64's x8, x16/x17 and
x18. The last of those rules is the one that matters most and is invisible
without it: r8–r11 ARE callee-saved under AAPCS32, but `computeFrame` saves
r4–r7, so a value pinned there is preserved by nobody and the damage appears in
the CALLER. `armFrameSaved` is read by both the pin check and the frame
computation, so "the pin is legal" and "the prologue saves it" cannot drift —
including under `-d:arkhamStress`, where the allocator's pools shrink but a body
that names its own registers still needs them saved.

Conditions are `(ite <flag> …)` as on x86-64, and `targets` is availability in
the ASSEMBLER rather than in the silicon: Cortex-M maps all eight of Arm's
conditions (`condOfFlagM`), AArch64's `genIteA64` implements the zero flag alone,
and parity is x86-64's — no Arm profile has such a bit. `Ctz` is the one row that
becomes two instructions (`rbit` + `clz`), which is what Arm's count-trailing-zeros
IS; both write the destination the body named and no register it did not declare.

Tests: `tests/arkham/assembler_a64.c.nif` (the `linux_arm64` qemu pass, the macOS
native pass, and both a64 stress levels), `tests/arkham_m/assembler_m.c.nif`
(Cortex-M under QEMU), and `tests/arkham_m/err_asm_*.c.nif` for the five
target-specific rejections.

Three things landed WITH the Arm mode that the x86-64 one still lacks, each of
them a §8 operand or statement the design always specified:

- **Two-address arithmetic on Arm.** `add`/`sub`/`and`/`or`/`xor`/`shl`/`shr`/
  `sar`/`neg` claim both Arm targets, in `.assembler` bodies and in ordinary
  procs. The allocated path serves a SPILLED destination through a staging bridge
  (load, operate, store back), because Arm is a load/store machine and a
  rejection there would fire only under register pressure — the worst shape a
  diagnostic can have. `not` stays x86-64: Arm spells it `mvn`, a two-OPERAND
  instruction in nifasm's Cortex-M tag set and absent from the AArch64 one.
  `inc`/`dec` stay because Arm has no such instruction — it writes `add #1`.
- **`addr` of a global or a `{.stack.}` local**: `(adr D sym)` and `(lea D slot)`.
  A register operand is refused — it has no address, and spilling it to give it
  one is the materialisation this mode exists to refuse.
- **A void row with operands** — `bkpt #imm8`, with a new operand pattern
  `ptImmLit` for the field the INSTRUCTION encodes. This is what lets a
  semihosting call be written in Nimony: the row is the trap, the body says what
  is in r0 and r1, and the answer that comes back in r0 is something no column
  here can describe — which is the whole reason such a call needs `.assembler`.

The flags rule is Arm's alone. `defs` on the arithmetic rows is x86-64's truth:
Arm has a flag-setting and a non-flag-setting form of each, and Thumb's narrow
encodings set the flags without being asked, so what is in NZCV after an `add`
depends on which registers the body pinned. The Arm mode therefore requires a
flag read to be the `if` IMMEDIATELY after its `cmp` (`asmIf`); a label, a
branch, a loop boundary and every other instruction invalidate it.
...
- **Globals on x86-64.** The Arm mode takes `addr` of one; the x86-64 mode still
  accepts only locals and parameters. `p = addr(g)` is one `lea`, but a pointer is
