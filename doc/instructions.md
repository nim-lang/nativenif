...
* **`(ldaxr …)`, `(stlxr …)`, `(ldar …)`, `(stlr …)`** — whether `lr.w`/`sc.w`
  reuse AArch64's exclusive spellings or mint their own. These are the only rows
  on this list that would *move* tag ids, since all four are `A64Inst`-only
  today and a second enum would lift them out of the late block.
...
* **`(clrex)`** — RV32 has no counterpart; a reservation is broken implicitly.
