...
(Thumb-2 / Cortex-M), `RvInst` (RV32IMAFD), `AvrInst` (AVR / avr5), or several of them for the
...
## RV32 and the tags it does not have

RV32 mints no register spellings and, so far, no mnemonics: every `RvInst` and
`RvReg` member above comes from a row that already named another target. That is
not thrift, it is the property that makes the target cheap to add — a row gaining
a *second* enum keeps its position in `genTags`' ordering, so the whole table's
tag ids are unmoved by this target's existence.

The register file is AArch64's, reinterpreted. `(x0)`..`(x30)` are RISC-V's
`x0`..`x30`, `(sp)` is `x2`, and `x31` is simply not mapped — 31 slots already
exceed what the allocator asks for, and the abstract `Reg` enum stops at `R30`.
The FP file is `(d0)`..`(d31)` and `(s0)`..`(s31)`, the same double/single views
of one register that AArch64 spells, and therefore the spelling `AsmBuf.freg`
already emits from a width. Deliberately **not** RISC-V's ABI names: `s0`..`s11`
are integer saved registers there, and one tag meaning two different register
*files* is a worse hazard than one meaning two machines.

These rows are still outstanding, each waiting on a decision the encoder has to
make first rather than on transcription. (Written as a list and not as a table
because `gen_instructions` reads *every* line beginning with `|` as a tag row —
a second table anywhere in this file is parsed as vocabulary.)

* **`(gload …)`, `(gstore …)`** — whether a global's address folds into the access
  that uses it. RISC-V spells the fold as an `auipc` carrying `%pcrel_hi` plus a
  load carrying `%pcrel_lo` *of a label pointing back at that `auipc`*, a
  relocation pairing nothing here does yet. Until then `PcRelGlobalFold` is
  absent and `(adr)` + `(ldr)` covers it.
* **`(ldar …)`, `(stlr …)`** — a plain load or store plus a `(dmb)` covers both,
  so RV32 needs no row of its own. (`(ldaxr …)`/`(stlxr …)` were the other half of
  this question and are answered: RV32 mints `(lrw …)`/`(scw …)` rather than
  reusing them, because a second enum on an `A64Inst`-only row lifts it out of the
  late block and shifts every shared id after it.)
* **`(clz …)`, `(rbit …)`, `(rev …)`** — Zbb. RV32I has no encoding for any of the
  three, so this is a question about the baseline ISA rather than about a
  spelling.
* **`(csellt …)` and the `csel*`/`cset*` family** — Zicond, also absent from the
  baseline. So `CondSelect` stays absent and a select lowers to the branch
  diamond Cortex-M already ships on.
* **`(tailcall …)`, `(popframe)`** — deferred rather than blocked: `TailCall` is
  off for the first landing.
* **`(clrex)`** — RV32 has no counterpart; a reservation is broken implicitly, so
  the compare-exchange failure path simply omits it.
...
| `(arch x64/arm64)`     | NifasmDecl                  | architecture pragma: `x64`, `linux_arm64`, `arm64`, `win_x64`, `win_arm64`, `cortex_m`, `riscv32` |
...
| `(prepare S ...)`      | X64Inst, A64Inst, MInst, RvInst, AvrInst | prepare block for function call |
| `(mov D S)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | move instruction |
| `(lea D S)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | load effective address |
...
| `(add D S)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | add instruction |
| `(sub D S)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | subtract instruction |
| `(mul S)`              | X64Inst, A64Inst, MInst, RvInst | unsigned multiply |
| `(imul D S)`           | X64Inst                  | signed multiply |
| `(div D S R)`          | X64Inst                  | unsigned divide |
| `(idiv D S R)`         | X64Inst                  | signed divide |
| `(sdiv D S)`           | A64Inst, MInst, RvInst | signed divide |
| `(udiv D S)`           | A64Inst, MInst, RvInst | unsigned divide |
| `(smulh D S)`          | A64Inst                  | signed multiply high (top 64 bits of D*S) |
| `(umulh D S)`          | A64Inst                  | unsigned multiply high (top 64 bits of D*S) |
| `(add3 D A B)`         | A64Inst, MInst, RvInst | 3-operand add (D = A + B) |
| `(sub3 D A B)`         | A64Inst, MInst, RvInst | 3-operand subtract (D = A - B) |
| `(mul3 D A B)`         | A64Inst, MInst, RvInst | 3-operand multiply (D = A * B) |
| `(and3 D A B)`         | A64Inst, MInst, RvInst | 3-operand bitwise and (D = A and B) |
| `(orr3 D A B)`         | A64Inst, MInst, RvInst | 3-operand bitwise or (D = A or B) |
| `(eor3 D A B)`         | A64Inst, MInst, RvInst | 3-operand bitwise xor (D = A xor B) |
| `(lsl3 D A B)`         | A64Inst, MInst, RvInst | 3-operand logical shift left (D = A shl B) |
| `(lsr3 D A B)`         | A64Inst, MInst, RvInst | 3-operand logical shift right (D = A shr B) |
| `(asr3 D A B)`         | A64Inst, MInst, RvInst | 3-operand arithmetic shift right (D = A sar B) |
| `(addw D S)`           | A64Inst, MInst, RvInst | 32-bit add (W-form, result zero-extended) |
| `(subw D S)`           | A64Inst, MInst, RvInst | 32-bit subtract (W-form, result zero-extended) |
| `(mulw D S)`           | A64Inst, MInst, RvInst | 32-bit multiply (W-form, result zero-extended) |
...
| `(and D S)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | bitwise and |
| `(or D S)`             | X64Inst, AvrInst | bitwise or |
| `(orr D S)`            | A64Inst, MInst, RvInst | bitwise or |
| `(xor D S)`            | X64Inst, AvrInst | bitwise xor |
| `(eor D S)`            | A64Inst, MInst, RvInst | bitwise xor |
| `(shl D S)`            | X64Inst                  | shift left |
| `(lsl D S)`            | A64Inst, MInst, RvInst | logical shift left |
| `(shr D S)`            | X64Inst                  | shift right |
| `(lsr D S)`            | A64Inst, MInst, RvInst | logical shift right |
| `(sal D S)`            | X64Inst                  | shift arithmetic left |
| `(sar D S)`            | X64Inst                  | shift arithmetic right |
| `(asr D S)`            | A64Inst, MInst, RvInst | arithmetic shift right |
| `(inc O)`              | X64Inst, AvrInst | increment |
| `(dec O)`              | X64Inst, AvrInst | decrement |
| `(neg O)`              | X64Inst, A64Inst, MInst, RvInst, AvrInst | negate |
...
| `(cmp D S)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | compare |
...
| `(call T ...)`         | X64Inst, A64Inst, MInst, RvInst, AvrInst | function call marker inside prepare |
| `(extcall)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | external call marker inside prepare |
...
| `(ret)`                | X64Inst, A64Inst, MInst, RvInst, AvrInst | return instruction |
| `(push O)`             | X64Inst, AvrInst | push to stack |
| `(pop O)`              | X64Inst, AvrInst | pop from stack |
| `(nop)`                | X64Inst, A64Inst, MInst, RvInst, AvrInst | no operation |
| `(syscall)`            | X64Inst                  | system call |
| `(svc N)`              | A64Inst                  | supervisor call (system call) |
| `(adr D L)`            | A64Inst, MInst, RvInst, AvrInst | load address of label |
| `(ldr D S)`            | A64Inst, MInst, RvInst | load register |
| `(str D S)`            | A64Inst, MInst, RvInst | store register |
| `(stp D1 D2 S)`        | A64Inst                  | store pair |
| `(ldp D1 D2 S)`        | A64Inst                  | load pair |
| `(b L)`                | A64Inst, MInst, RvInst, AvrInst | branch (unconditional jump) |
| `(bl L)`               | A64Inst, MInst, RvInst, AvrInst | branch with link (function call) |
| `(beq L)`              | A64Inst, MInst, RvInst, AvrInst | branch if equal |
| `(bne L)`              | A64Inst, MInst, RvInst, AvrInst | branch if not equal |
| `(blt L)`              | A64Inst, MInst, RvInst, AvrInst | branch if less than (signed) |
| `(ble L)`              | A64Inst, MInst, RvInst, AvrInst | branch if less or equal (signed) |
| `(bgt L)`              | A64Inst, MInst, RvInst, AvrInst | branch if greater than (signed) |
| `(bge L)`              | A64Inst, MInst, RvInst, AvrInst | branch if greater or equal (signed) |
| `(blo L)`              | A64Inst, MInst, RvInst, AvrInst | branch if lower (unsigned <) |
| `(bls L)`              | A64Inst, MInst, RvInst, AvrInst | branch if lower or same (unsigned <=) |
| `(bhi L)`              | A64Inst, MInst, RvInst, AvrInst | branch if higher (unsigned >) |
| `(bhs L)`              | A64Inst, MInst, RvInst, AvrInst | branch if higher or same (unsigned >=) |
| `(cbz S L)`            | A64Inst, MInst, RvInst | branch to L if S is zero (no flags read) |
| `(cbnz S L)`           | A64Inst, MInst, RvInst | branch to L if S is non-zero (no flags read) |
...
| `(lab L)`              | X64Inst, A64Inst, MInst, RvInst, AvrInst | label definition |
| `(ite ...)`            | X64Inst, A64Inst, MInst, RvInst, AvrInst | if-then-else structure |
| `(loop ...)`           | X64Inst, A64Inst, MInst, RvInst, AvrInst | loop structure |
| `(stmts ...)`          | X64Inst, A64Inst, MInst, RvInst, AvrInst | statement block |
| `(cfvar D)`            | NifasmDecl                  | control flow variable declaration |
| `(jtrue ...)`          | X64Inst, A64Inst, MInst, RvInst, AvrInst | set control flow variable(s) to true |
...
| `(kill S)`             | X64Inst, A64Inst, MInst, RvInst, AvrInst | kill variable |
...
| `(x0)`               | A64Reg, RvReg | register x0 |
| `(x1)`               | A64Reg, RvReg | register x1 |
| `(x2)`               | A64Reg, RvReg | register x2 |
| `(x3)`               | A64Reg, RvReg | register x3 |
| `(x4)`               | A64Reg, RvReg | register x4 |
| `(x5)`               | A64Reg, RvReg | register x5 |
| `(x6)`               | A64Reg, RvReg | register x6 |
| `(x7)`               | A64Reg, RvReg | register x7 |
| `(x8)`               | A64Reg, RvReg | register x8 |
| `(x9)`               | A64Reg, RvReg | register x9 |
| `(x10)`              | A64Reg, RvReg | register x10 |
| `(x11)`              | A64Reg, RvReg | register x11 |
| `(x12)`              | A64Reg, RvReg | register x12 |
| `(x13)`              | A64Reg, RvReg | register x13 |
| `(x14)`              | A64Reg, RvReg | register x14 |
| `(x15)`              | A64Reg, RvReg | register x15 |
| `(x16)`              | A64Reg, RvReg | register x16 |
| `(x17)`              | A64Reg, RvReg | register x17 |
| `(x18)`              | A64Reg, RvReg | register x18 |
| `(x19)`              | A64Reg, RvReg | register x19 |
| `(x20)`              | A64Reg, RvReg | register x20 |
| `(x21)`              | A64Reg, RvReg | register x21 |
| `(x22)`              | A64Reg, RvReg | register x22 |
| `(x23)`              | A64Reg, RvReg | register x23 |
| `(x24)`              | A64Reg, RvReg | register x24 |
| `(x25)`              | A64Reg, RvReg | register x25 |
| `(x26)`              | A64Reg, RvReg | register x26 |
| `(x27)`              | A64Reg, RvReg | register x27 |
| `(x28)`              | A64Reg, RvReg | register x28 |
| `(x29)`              | A64Reg, RvReg | register x29 |
| `(x30)`              | A64Reg, RvReg | register x30 |
| `(sp)`               | A64Reg, MReg, RvReg | stack pointer |
...
| `(yield)`            | A64Inst, MInst, RvInst | spin-wait hint (`hint #1`). One row for both Arm profiles because it is one instruction: `YIELD` is the same architectural hint on AArch64 and on ARMv7-M, and on both it is defined to execute as a `nop` where the implementation has nothing to do with it. That is what makes it safe to emit unconditionally — no feature test, no `when` at the call site |
| `(d0)`             | A64Reg, RvReg | fp register d0 |
| `(d1)`             | A64Reg, RvReg | fp register d1 |
| `(d2)`             | A64Reg, RvReg | fp register d2 |
| `(d3)`             | A64Reg, RvReg | fp register d3 |
| `(d4)`             | A64Reg, RvReg | fp register d4 |
| `(d5)`             | A64Reg, RvReg | fp register d5 |
| `(d6)`             | A64Reg, RvReg | fp register d6 |
| `(d7)`             | A64Reg, RvReg | fp register d7 |
| `(d8)`             | A64Reg, RvReg | fp register d8 |
| `(d9)`             | A64Reg, RvReg | fp register d9 |
| `(d10)`            | A64Reg, RvReg | fp register d10 |
| `(d11)`            | A64Reg, RvReg | fp register d11 |
| `(d12)`            | A64Reg, RvReg | fp register d12 |
| `(d13)`            | A64Reg, RvReg | fp register d13 |
| `(d14)`            | A64Reg, RvReg | fp register d14 |
| `(d15)`            | A64Reg, RvReg | fp register d15 |
| `(d16)`            | A64Reg, RvReg | fp register d16 |
| `(d17)`            | A64Reg, RvReg | fp register d17 |
| `(d18)`            | A64Reg, RvReg | fp register d18 |
| `(d19)`            | A64Reg, RvReg | fp register d19 |
| `(d20)`            | A64Reg, RvReg | fp register d20 |
| `(d21)`            | A64Reg, RvReg | fp register d21 |
| `(d22)`            | A64Reg, RvReg | fp register d22 |
| `(d23)`            | A64Reg, RvReg | fp register d23 |
| `(d24)`            | A64Reg, RvReg | fp register d24 |
| `(d25)`            | A64Reg, RvReg | fp register d25 |
| `(d26)`            | A64Reg, RvReg | fp register d26 |
| `(d27)`            | A64Reg, RvReg | fp register d27 |
| `(d28)`            | A64Reg, RvReg | fp register d28 |
| `(d29)`            | A64Reg, RvReg | fp register d29 |
| `(d30)`            | A64Reg, RvReg | fp register d30 |
| `(d31)`            | A64Reg, RvReg | fp register d31 |
| `(s0)`             | A64Reg, MReg, RvReg | fp register s0 |
| `(s1)`             | A64Reg, MReg, RvReg | fp register s1 |
| `(s2)`             | A64Reg, MReg, RvReg | fp register s2 |
| `(s3)`             | A64Reg, MReg, RvReg | fp register s3 |
| `(s4)`             | A64Reg, MReg, RvReg | fp register s4 |
| `(s5)`             | A64Reg, MReg, RvReg | fp register s5 |
| `(s6)`             | A64Reg, MReg, RvReg | fp register s6 |
| `(s7)`             | A64Reg, MReg, RvReg | fp register s7 |
| `(s8)`             | A64Reg, MReg, RvReg | fp register s8 |
| `(s9)`             | A64Reg, MReg, RvReg | fp register s9 |
| `(s10)`            | A64Reg, MReg, RvReg | fp register s10 |
| `(s11)`            | A64Reg, MReg, RvReg | fp register s11 |
| `(s12)`            | A64Reg, MReg, RvReg | fp register s12 |
| `(s13)`            | A64Reg, MReg, RvReg | fp register s13 |
| `(s14)`            | A64Reg, MReg, RvReg | fp register s14 |
| `(s15)`            | A64Reg, MReg, RvReg | fp register s15 |
| `(s16)`            | A64Reg, MReg, RvReg | fp register s16 |
| `(s17)`            | A64Reg, MReg, RvReg | fp register s17 |
| `(s18)`            | A64Reg, MReg, RvReg | fp register s18 |
| `(s19)`            | A64Reg, MReg, RvReg | fp register s19 |
| `(s20)`            | A64Reg, MReg, RvReg | fp register s20 |
| `(s21)`            | A64Reg, MReg, RvReg | fp register s21 |
| `(s22)`            | A64Reg, MReg, RvReg | fp register s22 |
| `(s23)`            | A64Reg, MReg, RvReg | fp register s23 |
| `(s24)`            | A64Reg, MReg, RvReg | fp register s24 |
| `(s25)`            | A64Reg, MReg, RvReg | fp register s25 |
| `(s26)`            | A64Reg, MReg, RvReg | fp register s26 |
| `(s27)`            | A64Reg, MReg, RvReg | fp register s27 |
| `(s28)`            | A64Reg, MReg, RvReg | fp register s28 |
| `(s29)`            | A64Reg, MReg, RvReg | fp register s29 |
| `(s30)`            | A64Reg, MReg, RvReg | fp register s30 |
| `(s31)`            | A64Reg, MReg, RvReg | fp register s31 |
| `(fmov D S)`        | A64Inst, MInst, RvInst | fp move (reg-reg / gpr<->fp bitcast) |
| `(fadd D S)`        | A64Inst, MInst, RvInst | fp add (D = D + S) |
| `(fsub D S)`        | A64Inst, MInst, RvInst | fp subtract (D = D - S) |
| `(fmul D S)`        | A64Inst, MInst, RvInst | fp multiply (D = D * S) |
| `(fdiv D S)`        | A64Inst, MInst, RvInst | fp divide (D = D / S) |
| `(fneg D)`          | A64Inst, MInst, RvInst | fp negate (D = -D) |
| `(fcmp D S)`        | A64Inst, MInst, RvInst | fp compare |
| `(fldr D S)`        | A64Inst, MInst, RvInst | fp load register |
| `(fstr D S)`        | A64Inst, MInst, RvInst | fp store register |
| `(scvtf D S)`       | A64Inst, MInst, RvInst | signed int -> fp convert |
| `(ucvtf D S)`       | A64Inst, MInst, RvInst | unsigned int -> fp convert |
| `(fcvtzs D S)`      | A64Inst, MInst, RvInst | fp -> signed int convert (toward zero) |
| `(fcvtzu D S)`      | A64Inst, MInst, RvInst | fp -> unsigned int convert (toward zero) |
...
| `(ldrb D B I)`      | A64Inst, MInst, RvInst | load byte (zero-extend), register offset [B,I] |
| `(strb D B I)`      | A64Inst, MInst, RvInst | store low byte, register offset [B,I] |
| `(rebind D T S)`    | X64Inst, A64Inst, MInst, RvInst, AvrInst | bind a phys reg to a typed name, killing its prior tenant |
| `(withreg D T S ...)` | X64Inst, A64Inst, MInst, RvInst, AvrInst | block-scoped rebind; auto-killed at block end |
...
| `(scope ...)`         | X64Inst, A64Inst, MInst, RvInst, AvrInst | statement block with a reclaimable stack-slot arena: `(s)` locals declared inside are freed at scope end so sibling scopes reuse the frame bytes |
...
| `(bkpt N)`           | MInst, AvrInst | breakpoint / semihosting call; `(bkpt 171)` is `bkpt #0xAB`, the ARM semihosting entry on M-profile (operation in r0, parameter block in r1, result back in r0) |
...
| `(andi D K)`           | AvrInst | AVR: `(andi D K)`, and with an 8-bit immediate, r16..r31 only. RV32: `(andi D A K)`, three-operand with a 12-bit signed immediate. Same spelling, different machine — `(arch …)` decides, exactly as it does for `(r0)` and `(add D S)` |
| `(ori D K)`            | AvrInst | AVR: `(ori D K)`, or with an 8-bit immediate, r16..r31 only. RV32: `(ori D A K)`, three-operand with a 12-bit signed immediate. See `(andi …)` |
