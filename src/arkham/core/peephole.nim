...
##     itself. Needs `Target.immAnyDest`.
...
##  3. `(and|or|xor D S)` + `(test D D)` ⇒ the test dropped: the logical op
##     already left exactly the flags it would have set. Needs
##     `Target.logicalSetsFlags`. 4.29 M of executed `test` on nifbench's
##     `parse` against gcc's zero.
##
## Every mnemonic test goes through `tagOf`, which decodes to the generated
## `TagEnum` the rest of the assembler is written against — not to a string, and
## not to `cursorTagId` (asm-NIF escapes any tag past the 9-bit field). What
## each rule asks of the MACHINE is a field of `Target`, stated once, rather
## than an `arch` string tested at the point of use.
##
...
import nifcore
import "../../nifasm/core" / [tags, tagpool]

when defined(arkhamPeepDbg):
  import std / syncio
  var dbgCand*, dbgPair*, dbgTest*: int

template tagOf(c: Cursor): TagEnum =
  ## The node's mnemonic, as the generated `TagEnum` the rest of the assembler
  ## is written against. Two things this deliberately is NOT:
  ##
  ##  * not a string. `tagName(…) == "mov"` costs a lookup and a compare per
  ##    node, and a typo is a rule that silently never fires. These are the same
  ##    ordinals `X64Inst`/`A64Inst` are built on — `tagpool.createAsmTagPool`
  ##    seeds the buffer so the ids ARE the enum ordinals, and asserts it — so a
  ##    set of them is a machine-word test and a misspelling is a compile error.
  ##  * not `cursorTagId`. asm-NIF's vocabulary overflows NIF's 9-bit tag field,
  ##    so a mnemonic numbered past 511 arrives as `(other <id> …)` whose
  ##    `cursorTagId` is the escape. `resolvedTagId` unfolds it.
  (if c.kind == TagLit: cast[TagEnum](uint32(resolvedTagId(c))) else: InvalidTagId)

proc operands(c: Cursor): Cursor {.inline.} =
  ## A cursor on operand 0, whether or not the mnemonic needed the escape.
  ## `sub` alone hands back the escaped id as if it were operand 0 — the one
  ## way a rule matched through `tagOf` could still read a node wrongly.
  result = sub(c)
  if isEscapedTag(c): inc result

type
  Target* = object
    ## What the machine permits. Every field is a fact about the ISA, not a
    ## tuning knob, and each is stated where the rule that needs it is.
    gprs*: set[TagEnum]
      ## The target's own tags for the registers a memory base may live in,
      ## taken from the shim that WROTE them into the buffer rather than from a
      ## list kept in step with it.
    immAnyDest*: bool
      ## An immediate may go into ANY `mov` destination. x86-64 can (`mov r/m,
      ## imm32`); AArch64 cannot — a store there is `str <reg>, [addr]` with no
      ## immediate form, so the constant has to reach a register anyway and
      ## folding would move the work rather than remove it. A bare-symbol
      ## destination is a stack slot or a register home depending on a `(var …)`
      ## far above and the two cannot be told apart here, so the target answers
      ## for both.
    logicalSetsFlags*: bool
      ## `and`/`or`/`xor` write the condition flags as a side effect, and write
      ## exactly what `test D,D` would. True on x86; false on AArch64, where
      ## that is `ands` and a plain `and` writes nothing.
  ...
  Home = object
    ...
    gpr: bool
    reg: TagEnum     ## the home register's tag, so "does this instruction write
...
const
  BarrierTags = {ScopeTagId, LoopTagId, LabTagId, ProcTagId, StmtsTagId,
                 VarTagId, RebindTagId, ParamTagId, CallTagId, TailcallTagId,
                 SyscallTagId, ExtcallTagId, KillTagId}
  ...
  AddrTags = {MemTagId, AtTagId, DotTagId}
  ...
  ReadsFromSecond = {
    ...
    MovTagId, MovzxTagId, MovsxTagId, MovsdTagId, MovssTagId, MovapdTagId,
    MovupdTagId, MovupsTagId, MovdquTagId, MovfqTagId, MovfdTagId,
    MovwTagId, LeaTagId, AddTagId, SubTagId, ImulTagId, AndTagId, OrTagId,
    XorTagId, EorTagId, OrrTagId, AdcTagId, SbcTagId, ShlTagId, ShrTagId,
    SarTagId, SalTagId, RolTagId, RorTagId, RclTagId, RcrTagId, LslTagId,
    LsrTagId, AsrTagId, AddsdTagId, SubsdTagId, MulsdTagId, DivsdTagId,
    AddssTagId, SubssTagId, MulssTagId, DivssTagId, AddpdTagId, SubpdTagId,
    MulpdTagId, AddpsTagId, SubpsTagId, MulpsTagId, PunpcklqdqTagId,
    ShufpsTagId, Cvtsd2ssTagId, Cvtss2sdTagId, Cvtsi2sdTagId,
    Cvtsi2ssTagId, Cvttsd2siTagId, Cvttss2siTagId, BsfTagId, BsrTagId,
    PopcntTagId, BtTagId, BtsTagId, BtrTagId, BtcTagId, BswapTagId,
    MvnTagId, SxtbTagId, SxthTagId, UxtbTagId, UxthTagId, ClzTagId,
    RbitTagId, RevTagId, Add3TagId, Sub3TagId, Mul3TagId, And3TagId,
    Orr3TagId, Eor3TagId, Bic3TagId, Lsl3TagId, Lsr3TagId, Asr3TagId,
    Addw3TagId, Subw3TagId, Mulw3TagId, Adds3TagId, Subs3TagId, AddwTagId,
    SubwTagId, MulwTagId, MlsTagId, CmpTagId, TestTagId, ComisdTagId,
    ComissTagId, FcmpTagId, TstTagId}

  ReadsFromFirst = {CmpTagId, TestTagId, ComisdTagId, ComissTagId, FcmpTagId,
                    TstTagId, PushTagId}
  ...
  PureDestTags = {
    ...
    MovTagId, MovzxTagId, MovsxTagId, MovsdTagId, MovssTagId, MovapdTagId,
    MovupdTagId, MovupsTagId, MovdquTagId, MovfqTagId, MovfdTagId,
    MovwTagId, LeaTagId, Cvtsd2ssTagId, Cvtss2sdTagId, Cvtsi2sdTagId,
    Cvtsi2ssTagId, Cvttsd2siTagId, Cvttss2siTagId, Add3TagId, Sub3TagId,
    Mul3TagId, And3TagId, Orr3TagId, Eor3TagId, Bic3TagId, Lsl3TagId,
    Lsr3TagId, Asr3TagId, Addw3TagId, Subw3TagId, Mulw3TagId, Adds3TagId,
    Subs3TagId}

proc operandIsRead(op: TagEnum; i: int): bool {.inline.} =
  if i == 0: op in ReadsFromFirst
  else: op in ReadsFromSecond
...
proc collectOccs(buf: var TokenBuf; c: var Cursor; isWrite, inKill: bool;
  ...
  of TagLit:
    let op = tagOf(c)
    let isKill = op == KillTagId
    let pureDest = op in PureDestTags
    var i = 0
    c.loopInto:
...
proc noteDecl(buf: var TokenBuf; c: Cursor; homes: var Table[SymId, Home];
  ...
  let op = tagOf(c)
  let isRebind = op == RebindTagId
  if op notin {RebindTagId, VarTagId, ParamTagId}: return
  var b = operands(c)
  ...
  if homeC.kind == TagLit:
    ...
    let hb = operands(homeC)
    h.gpr = not hb.hasMore and tagOf(homeC) in gprs
    if h.gpr: h.reg = tagOf(homeC)
  homes[s] = h
...
proc movImmDest(buf: var TokenBuf; c: Cursor; dst: var SymId; imm: var Cursor): bool =
  ...
  if tagOf(c) != MovTagId: return false
  var b = operands(c)
  if not b.hasMore or b.kind != Symbol: return false
  dst = b.symId
  skip b
  if not b.hasMore: return false
  if b.kind == IntLit or b.kind == UIntLit or tagOf(b) == NilTagId:
    imm = b
    skip b
...
proc scanUse(buf: var TokenBuf; c: var Cursor; t, s: SymId; isRead, underCast: bool;
  ...
  of TagLit:
    let op = tagOf(c)
    if op in BarrierTags: r.bad = true
    let isAddr = op in AddrTags
    let isCast = op == CastTagId
    var i = 0
    c.loopInto:
      let childRead =
        if isAddr: not (op == AtTagId and i == AtScratchIndex)
        elif isCast: isRead
        else: operandIsRead(op, i)
...
proc movCopy(buf: var TokenBuf; c: Cursor; dst: var SymId;
             src: var CopySrc; gprs: set[TagEnum]): bool =
...
proc declSkippable(buf: var TokenBuf; c: Cursor; a, b: SymId; srcReg: TagEnum): bool =
  ...
  let op = tagOf(c)
  if op notin {VarTagId, RebindTagId}: return false
  var k = operands(c)
  ...
  let home = if op == RebindTagId: third else: second
  result = tagOf(home) != srcReg
...
proc writesReg(buf: var TokenBuf; c: Cursor; r: TagEnum; ctx: Ctx;
  ...
  ## here.
  if tagOf(c) in ReadsFromFirst: return false  # cmp/test/push write no register
  var b = operands(c)
  if not b.hasMore: return false
  # Peel a retyping cast; an address expression is a MEMORY destination and
  # writes no register.
  while tagOf(b) == CastTagId:
    var cb = operands(b)
...
proc passable(buf: var TokenBuf; c: Cursor; t: SymId; src: CopySrc;
  ...
  if declSkippable(buf, c, t, src.sym, srcReg):
    ...
    noteDecl(buf, c, seen, ctx.target.gprs)
    return true
  if tagOf(c) notin ReadsFromSecond: return false
...
proc tryRebase(buf: var TokenBuf; c: var Cursor; dest: var TokenBuf;
  ...
  var t: SymId
  var src: CopySrc
  if not movCopy(buf, c, t, src, ctx.target.gprs): return false
  ...
  # names mean.
  let srcReg = if src.raw: tagOf(src.node)
...
proc trNode(buf: var TokenBuf; c: var Cursor; dest: var TokenBuf; ctx: var Ctx) =
  if c.kind == TagLit:
    ...
    let isProc = tagOf(c) == ProcTagId
    let outer = ctx.limit
    if isProc:
      ctx.limit = cursorToPosition(buf, c) + subtreeWidth(c)
      ctx.homes.clear()
    noteDecl(buf, c, ctx.homes, ctx.target.gprs)
    dest.openTag c.cursorTagId
    c.loopInto:
...
proc logicalToSym(buf: var TokenBuf; c: Cursor; dst: var SymId): bool =
  ...
  ##
  ## `add`/`sub` are deliberately absent. They leave OF and CF from the
  ...
  ## three below need no such argument: their flag result IS `test`'s, for every
  ## flag, so no consumer can tell.
  if tagOf(c) notin {AndTagId, OrTagId, XorTagId}: return false
  var b = operands(c)
  if not b.hasMore or b.kind != Symbol: return false
  dst = b.symId
  skip b
...
proc testOfSym(buf: var TokenBuf; c: Cursor; s: SymId): bool =
  ...
  if tagOf(c) != TestTagId: return false
  var b = operands(c)
...
proc trList(buf: var TokenBuf; c: var Cursor; dest: var TokenBuf; ctx: var Ctx) =
  ...
  var aluDst: SymId
  if ctx.target.logicalSetsFlags and logicalToSym(buf, c, aluDst):
    # RULE 3. `(and D S)` + `(test D D)` ⇒ drop the test: the logical op already
    # left exactly the flags it would have set, so no consumer can tell. On
    # nifbench's `parse` that is 4.29 M of executed `test` against gcc's ZERO.
    #
    # The only thing allowed between the two is `(kill …)`, and it is what makes
    # the rule worth having: the emitters release the AND's sources right after
    # it, so
    #     (and `tmp3.0 `tmp6.0) (kill `tmp6.0) (test `tmp3.0 `tmp3.0)
    # is the common shape and strict adjacency finds only a quarter of the
    # sites. `genKill` releases a slot or a binding and emits no byte, so
    # "nothing touched the flags in between" stays true by inspection rather
    # than by analysis. Nothing else may intervene: a `(lab …)` is a join where
    # the flags arriving are some other path's.
    var la = c
    skip la
    var kills = 0
    while la.hasMore and tagOf(la) == KillTagId:
      skip la
      inc kills
    if la.hasMore and testOfSym(buf, la, aluDst):
      when defined(arkhamPeepDbg): inc dbgTest
      trNode(buf, c, dest, ctx)                # keep the ALU op
  ...
  var dst: SymId
  var immNode: Cursor
  if ctx.target.immAnyDest and movImmDest(buf, c, dst, immNode):
...
proc peephole*(buf: var TokenBuf; target: Target): int =
  ## Rewrite `buf` in place; returns the number of instructions removed. What
  ## each rule asks of the machine is stated on `Target`; rule 2 asks nothing
  ## beyond `gprs`, because every target addresses memory through a register.
  var ctx = Ctx(homes: initTable[SymId, Home](), target: target,
                limit: buf.len, folds: 0)
  block:
    var c = beginRead(buf)
    while c.hasMore:
