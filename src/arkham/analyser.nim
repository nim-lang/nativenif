...
proc analyseProc*(buf: var TokenBuf; procDecl: Cursor;
  ...
  #
  # `entryLeadingClobber` disables ArgResident wholesale: normally the `arkhamGlobalInit`
  # call is APPENDED to the module init proc's body — a proc with no parameters — but a
  # module with no init proc hangs it off the ENTRY instead (`initsGlobalsAtEntry`), where
  # it is INJECTED before the body (not in the analysed IR) and clobbers the caller-saved
  # arg registers before the first body use of argc/argv/envp. No param may stay in its
  # incoming register then.
  #
