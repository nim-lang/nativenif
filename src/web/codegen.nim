...
type
  ...
  ProcCtx = object
    ...
    symType: Table[SymId, Cursor] ## local/param symbol → its Leng type
    locals: Table[SymId, LocalSlot]
    retType: Cursor
    ...
    regLocals: seq[SymId]         ## `lkReg` locals, in declaration order
  ...
  WebGen* = object
    ...
    callTarget: Table[SymId, CallTarget] ## typenav needs a mutable copy
    globals: Table[SymId, Cursor]        ## symbol → gvar/const decl (foreign ones cached on use)
    tvars: Table[SymId, Cursor]
...
proc err(g: WebGen; msg: string) {.noreturn.} =
  ## A program the generator does not understand — an ordinary failure, not a
  ## broken invariant, so it is never an `assert`: see `diag.refuse`.
  refuse msg
...
proc spell(g: WebGen; sym: SymId): string {.inline.} =
  ## A Leng symbol's SPELLING — for a diagnostic, and for the one mangling that
  ## turns it into a JavaScript identifier (`irName`). Nothing else needs text.
  ...
  if initv.exprKind == AconstrC:
    ...
    ac.into:
      ...
      if arrT.kind == TagLit and arrT.typeKind in {PtrT, AptrT}:
        ...
        esz = typeSizeAlign(g.prog, elemT)[0]
      skip ac
      var n = 0
...
proc globalAddrOf(g: var WebGen; name: SymId): uint32 =
  ...
  if g.globalAddr.hasKey(canon): return g.globalAddr.getOrQuit(canon)
  let si = lookupSym(typeCtx(g), name)
  if si.cat notin {scGlobal, scTvar}:          # tvar: single-threaded target → a global
    err g, "not a global: " & g.spell(name)
  var d = si.decl
  var typ: Cursor = default(Cursor)
  var initv: Cursor = default(Cursor)
  var hasInit = false
  d.into:
    inc d                                      # name
    skip d                                     # pragmas
  ...
  if not g.canonDecl.hasKey(canon) or
      (hasInit and not declHasInit(g.canonDecl.getOrQuit(canon))):
...
proc strLitAddr(g: var WebGen; s: string): uint32 =
  if g.rodataAddr.hasKey(s): return g.rodataAddr.getOrQuit(s)
...
proc tableSlotOf(g: var WebGen; sym: SymId): uint32 =
  ...
  if g.tableSlot.hasKey(sym): return g.tableSlot.getOrQuit(sym)
...
proc fieldOffsetIn(g: var WebGen; objType: Cursor; field: SymId;
                   found: var bool): int =
...
proc dotOffset(g: var WebGen; baseType: Cursor; field: SymId; depth: int): int =
  ...
  while true:
    ...
    var oc = t
    var base: Cursor = default(Cursor)
    var hasBase = false
    oc.into:
      base = oc
    ...
    if lvl > 0:
      ...
      if found: return off
      if not hasBase: err g, "field not found: " & g.spell(field)
      t = resolveType(g.prog, base)
...
proc isAggregateGlobal(g: var WebGen; nm: SymId): bool =
      ...
      t.into:
        ...
        if floatTarget:
          ...
          of CharLit: result = cast[uint64](float64(ord(charLit(t))))
            ...
            result = 0
        elif t.kind == Symbol and lookupSym(typeCtx(g), t.symId).cat == scProc:
        ...
        elif t.kind == Symbol and (ptrTarget or isAggregateGlobal(g, t.symId)):
          ...
          result = uint64(globalAddrOf(g, t.symId))
        else:
          result = constScalarBits(g, t, ok)
...
proc serializeConstInto(g: var WebGen; bytes: var string; base: int;
  ...
  if v.kind == TagLit and v.exprKind == OconstrC:
    ...
    putLE(bytes, base, 0, WebPtrSize)
      ...
      lookupSym(typeCtx(g), v.symId).cat in {scGlobal, scTvar}:
    ...
    putLE(bytes, base, uint64(globalAddrOf(g, v.symId)), WebPtrSize)
  else:
    let sc = scalOf(g, rt)
    if sc.kind == skMem:
...
proc staticInit(g: var WebGen; decl: Cursor; typ, initv: var Cursor;
  ...
  if initv.kind == Symbol:
    let ic = lookupSym(typeCtx(g), initv.symId).cat
    ...
    t.into:
      ...
      staticInner = t.kind in {IntLit, UIntLit, CharLit, FloatLit, StrLit} or
        ...
        (ptrTarget and t.kind == Symbol and
         lookupSym(typeCtx(g), t.symId).cat in {scGlobal, scTvar}) or
        (t.kind == Symbol and lookupSym(typeCtx(g), t.symId).cat == scProc)
      while t.hasMore: skip t
    result = staticInner
...
proc serializeStatics(g: var WebGen) =
  ...
  while true:
    ...
    for (_, n) in round:
      var typ, initv: Cursor = default(Cursor)
      var hasInit = false
      if not staticInit(g, g.canonDecl.getOrQuit(n), typ, initv, hasInit): continue
      var bytes = ""
      serializeConstInto(g, bytes, 0, typ, initv)
      if bytes.len > 0:
        g.dataSegs.add (g.globalAddr.getOrQuit(n), bytes)
...
proc layoutProgram*(g: var WebGen) =
  ...
  # Sorted by the SPELLING, not by the id: a deterministic layout is the point,
  # and an id is an interning ORDER — which depends on what the generator
  # happened to look at first, i.e. on the emission it is supposed to fix.
  var names: seq[(string, SymId)] = @[]
  for n in g.globals.keys: names.add (g.spell(n), n)
  for n in g.tvars.keys: names.add (g.spell(n), n)
  sort names, proc (a, b: (string, SymId)): int = cmp(a[0], b[0])
  for (_, n) in names: discard globalAddrOf(g, n)
  serializeStatics(g)
...
proc createWebGen*(buf: var TokenBuf; inputPath: string; tags: TagPool;
  ...
  result.tvars = initTable[SymId, Cursor]()
  ...
  result.p.symType = initTable[SymId, Cursor]()
  result.p.locals = initTable[SymId, LocalSlot]()
...
proc irName(g: var WebGen; sym: SymId): SymId =
  ...
  ## JS reserved words.
  if g.irNameOf.hasKey(sym): return g.irNameOf.getOrQuit(sym)
  var base = "n_"
...
proc declType(g: var WebGen; nm: SymId): Cursor =
  ...
  let si = lookupSym(typeCtx(g), nm)
  if si.cat notin {scGlobal, scTvar}: err g, "not a global: " & g.spell(nm)
  var d = si.decl
  d.into:
    inc d                                      # name
    skip d                                     # pragmas
...
proc genSymAddr(g: var WebGen; c: Cursor) =
  ...
  if g.p.locals.hasKey(nm):
    let s = g.p.locals.getOrQuit(nm)
    case s.kind
    of lkReg: err g, "the register local `" & g.spell(nm) & "` has no address"
    ...
    else: err g, "not addressable: " & g.spell(nm)
...
proc genSymValue(g: var WebGen; c: Cursor) =
  ...
  if g.p.locals.hasKey(nm):
    let s = g.p.locals.getOrQuit(nm)
    let ty = g.p.symType.getOrQuit(nm)
    ...
    of scGlobal, scTvar:
      ...
      if isAggType(g, ty):
        ...
        g.outp.tree HLoad:
          g.outp.width widthOf(scalOf(g, ty))
          g.outp.numLit int64(globalAddrOf(g, nm))
    of scNone: err g, "unknown symbol: " & g.spell(nm)
...
proc isInheritedPart(g: var WebGen; objTy: Cursor; part: SymId): bool =
  ...
  while t.kind == TagLit and t.typeKind == ObjectT:
    ...
    if base.symId == part: return true
    t = resolveType(g.prog, base)
...
proc genCtorInto(g: var WebGen; destOff: int; c: Cursor) =
  ...
  t.into:
    ...
    var isFirst = true
    while t.hasMore:
      var val: Cursor = default(Cursor)
      var off = 0
      var vt: Cursor = default(Cursor)
      var header = false
...
proc collectNames(c: Cursor; taken: var HashSet[SymId]) =
  ## Every symbol in a subtree.
  case c.kind
  of Symbol: taken.incl c.symId
  of TagLit:
    var t = c
    t.into:
      while t.hasMore:
...
proc markTaken(c: Cursor; taken: var HashSet[SymId]) =
...
proc planNode(g: var WebGen; pl: var FramePlan; c: Cursor; needsTemp: bool) =
  ...
  if c.stmtKind == VarS:
    ...
    t.into:
      ...
      if typ.kind == DotToken:
        ...
        var v = t
        inc v
        if v.kind == DotToken: err g, "local `" & g.spell(nm) & "` has no type"
        typ = lengType(g, v)
      g.p.symType[nm] = typ
  ...
  var spliceBridges: seq[JsBridgeKind] = default(seq[JsBridgeKind])
  if c.exprKind == CallC:
    var tg = sub(c)
    if tg.kind == Symbol:
      var found = false
...
proc planFrame(g: var WebGen; body: Cursor; params: seq[(SymId, Cursor)];
               taken: HashSet[SymId]) =
...
proc closureThunk(g: var WebGen; c: Cursor): bool =
  ...
  t.into:
    let dstT = t
    skip t
    if t.kind == Symbol and not g.p.locals.hasKey(t.symId):
      let opSym = symId(t)
      if lookupSym(typeCtx(g), t.symId).cat == scProc:
...
proc genIndirectCall(g: var WebGen; target: Cursor; t: var Cursor) =
  ...
  if pt.cursorIsNil:
    err g, (if target.kind == Symbol: "indirect call through unknown symbol " &
                                          g.spell(target.symId)
            else: "indirect call through a non-proctype value")
  var retT: Cursor = default(Cursor)
  var paramsT: Cursor = default(Cursor)
  pt.into:
...
proc proctypeSig(g: var WebGen; pt: Cursor): (seq[Cursor], Cursor) =
  ...
  var params: seq[Cursor] = default(seq[Cursor])
  var p = sub(pt)
  skip p                                     # NAME
...
proc operandProcSym(g: var WebGen; t: Cursor): SymId =
  ...
  while c.kind == TagLit and c.exprKind in {AddrC, HaddrC}:
    inc c
  if c.kind == Symbol and lookupSym(typeCtx(g), c.symId).cat == scProc:
    result = symId(c)
  ...
  p.into:
    ...
    p.into:                                  # params
      while p.hasMore:
        ...
        var agg = false
        var bk = jbNone
        var ptq = default(Cursor)            # the proctype of a callback parameter
        q.into:
          inc q                              # name
          skip q                             # pragmas
          agg = isAggType(g, q)
...
proc declSignature(g: var WebGen; decl: Cursor): WebImport =
  ...
  result = default(WebImport)
  var d = decl
  d.into:
    inc d                                      # name
    var rt = default(Cursor)
    ...
    for p in ps: result.params.add p
    while d.hasMore: skip d
...
proc hostImport(g: var WebGen; sym: SymId; decl: Cursor): SymId =
  ...
  if g.importOf.hasKey(icName): return g.importOf.getOrQuit(icName)
...
proc genCallFrom(g: var WebGen; t: var Cursor; wantValue: bool) =
  ...
  var nm = NoTypeSym
  var nmSym = SymId(0)
  var ct: CallTarget = default(CallTarget)
  var known = false
  if t.kind == Symbol:
    nm = t.symId
    nmSym = symId(t)
  ...
  if known and ct.syscall:
    ...
    var base = g.spell(nm)
    if ct.asmName != NoTypeSym: base = g.prog.cNameOfAsmName(ct.asmName)
    ...
    if found and not hasBody(decl) and hasPragma(decl, ImportjsP):
      if g.target != wtJs:
        err g, "`importjs` proc `" & g.spell(nm) & "` has no wasm lowering"
      ...
      if not (found and g.hostImports):
        err g, "extern `" & g.spell(nm) & "` has no host binding (bodyless importc)"
      ...
      if not found: err g, "no body to call: " & g.spell(nm)
...
proc genVar(g: var WebGen; c: Cursor) =
  ...
  t.into:
    ...
    if t.hasMore and t.kind != DotToken:
      ...
      hasInit = true
    while t.hasMore: skip t
  if not g.p.locals.hasKey(nm): err g, "internal: unplanned local `" & g.spell(nm) & "`"
  let sl = g.p.locals.getOrQuit(nm)
  case sl.kind
  of lkReg:
    ...
    if hasInit:
      let w = widthOf(scalOf(g, g.p.symType.getOrQuit(nm)))
    ...
    if isAggType(g, ty):
      ...
      g.outp.tree ExprStmt:
        g.outp.tree HStore:
          ...
          slotAddr(g, sl.off)
          g.genExprCoerced(initv, w)
  of lkPtr: err g, "internal: `" & g.spell(nm) & "` is a parameter, not a local"
...
proc lvalueType(g: var WebGen; c: Cursor): Cursor =
  ...
  if c.kind == Symbol:
    let nm = c.symId
    if g.p.locals.hasKey(nm): result = g.p.symType.getOrQuit(nm)
...
proc assignTo(g: var WebGen; dst, src: Cursor) =
  ...
  if dst.kind == Symbol and g.p.locals.hasKey(dst.symId) and
      g.p.locals.getOrQuit(dst.symId).kind == lkReg:
    let nm = dst.symId
    g.outp.tree Assign:
      g.outp.symUse irName(g, nm)
      g.genExprCoerced(src, widthOf(scalOf(g, g.p.symType.getOrQuit(nm))))
    return
...
proc storeTempTo(g: var WebGen; dst: Cursor; tmp: SymId; w: WidthCode) =
  ...
  if dst.kind == Symbol and g.p.locals.hasKey(dst.symId) and
      g.p.locals.getOrQuit(dst.symId).kind == lkReg:
    g.outp.tree Assign:
      g.outp.symUse irName(g, dst.symId)
      g.outp.symUse tmp
    return
  let ty = lvalueType(g, dst)
  if isAggType(g, ty):
...
proc genKeepovf(g: var WebGen; c: Cursor) =
  ...
  t.into:
    ...
    while t.hasMore: skip t
    var a = arith
    var opKind: LengExpr = default(LengExpr)
    var typ, lhs, rhs: Cursor = default(Cursor)
    a.into:
...
proc caseRangeTest(g: var WebGen; w: WidthCode; scrutinee: SymId; r: Cursor) =
  ...
  if r.kind == TagLit and r.substructureKind == RangeU:
    var lo, hi: Cursor = default(Cursor)
    var t = r
    t.into:
      lo = t
      skip t
...
proc genCaseBranch(g: var WebGen; w: WidthCode; scrutinee: SymId;
  ...
  ## `(range LO HI)` at all.
  var rs: seq[Cursor] = default(seq[Cursor])
  var t = branches[i][0]
  t.into:
    while t.hasMore:
      rs.add t
...
proc genCase(g: var WebGen; c: Cursor) =
  ...
  var elseBody: Cursor = default(Cursor)
  var scrutinee: Cursor = default(Cursor)
  var t = c
  t.into:
    scrutinee = t
    skip t
  ...
  t.into:
    while t.hasMore:
      ...
      if pl != SymId(0) and padJoin.hasKey(pl):
        ...
        g.outp.symUse padJoin.getOrQuit(pl)
        g.outp.closeTag
        ...
        if g.p.labs.len == 0 or g.p.labs[^1] != padJoin.getOrQuit(pl):
...
proc lowerProc(g: var WebGen; sym: SymId; decl: Cursor) =
  ...
  g.p = ProcCtx(irName: irName(g, sym),
                symType: initTable[SymId, Cursor](),
                locals: initTable[SymId, LocalSlot]())
  let body = procBody(decl)
  var importcN, exportcN = ""
  var params: seq[(SymId, Cursor)] = @[]
  var t = decl
  t.into:
    inc t                                      # the name
    t.into:                                    # params
      while t.hasMore:
        var pname = NoTypeSym
        var ptyp: Cursor = default(Cursor)
        var q = t
        q.into:
          pname = q.symId
          inc q
          if hasPragmaIn(q, RegisterP):
            ...
            err g, "`{.register.}` parameter `" & g.spell(pname) & "` in `" &
                  g.spell(sym) & "`"
          skip q                               # pragmas
          ptyp = q
          while q.hasMore: skip q
  ...
  var taken: HashSet[SymId] = default(HashSet[SymId])
  ...
  if g.p.frameSize > 0:
    ...
    for (pn, pt) in params:
      let sl = g.p.locals.getOrQuit(pn)
      if sl.kind == lkSlot:
        g.outp.tree ExprStmt:
          g.outp.tree HStore:
  ...
  for nm in g.p.regLocals:
    locals.add (irName(g, nm), widthOf(scalOf(g, g.p.symType.getOrQuit(nm))))
...
proc generate*(buf: var TokenBuf; inputPath: string; tags: TagPool;
  ...
  layoutProgram(g)
  var entryDecl: Cursor = default(Cursor)
  var haveEntry = false
  for pi in g.prog.procs:
    if pi.isEntry:
      var nc = pi.decl
  ...
  g.top.openTree Top
  let entrySym = g.entrySym            # a field of `g` cannot also be an
  ensureProc(g, entrySym, entryDecl)   # argument of a call that mutates it
  ...
  var rootDecls: seq[(SymId, Cursor)] = @[]     # the same procs, with their decls
  for pi in g.prog.procs:
    ...
    if exportcN.len > 0 and importcN.len == 0:
      ...
      rootDecls.add (sym, pi.decl)
  # `ensureProc` GROWS `g`, and the walk above reads `g.prog.procs` — so the
  # roots are collected first and reached afterwards, never from inside it.
  for (sym, decl) in rootDecls:
    ensureProc(g, sym, decl)
